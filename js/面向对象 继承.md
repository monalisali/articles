...
从控制台信息可以知道: 在`Solider.__proto__ `中多了两个属性:name,skin, 它们是在`Solider.prototype = new Human({name:'i am human', skin: 'human color'});`时被赋值的。
...
但是这种做法太麻烦了，JS在新版本中提供了更简便的: `object.create()`
...
2. `typeof Human` 返回的是"function", 就是说即使使用了关键字class, Human其实还是一个funcition。但是调用所谓的函数Human是又会报错，
    `Human.call()`报错：Class constructor Human cannot be invoked without 'new'。 就是说，JS认为因为Human使用了class，就变成一个特殊的函数了，不可以调用！
