...
用`()`来调用函数的形式,如`fn()`其实是阉割版的`call()`。 阉割在只能传递参数，而无法传递this，this的值是什么只能让浏览器自己猜了。
...
## 为什么this必须是对象
还是上例，现在这样调用`fn.call(10,1)` 显然10就是this，打印出来后发现this为Number对象。那为什么传递的明明是10，但是this为什么会变成Number对象呢？
